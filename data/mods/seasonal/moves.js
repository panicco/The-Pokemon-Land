...
exports.BattleMovedex = {
	...
	lolroom: {
		...
		effect: {
			onStart: function (pokemon) {
				...
				let newatk = pokemon.storedStats.spd;
				let newdef = pokemon.storedStats.spa;
	...
	wonderbark: {
		...
		onHit: function (pokemon, source) {
			...
			for (let i = 0; i < pokemon.moveset; i++) {
			...
			onHit: function (target, source) {
				if (this.random(20) === 10) {
					...
					if (status === 'frz') {
						...
						if (freeze && target.trySetStatus('frz') && source.name === 'xjoelituh') {
							prompt = true;
						}
					} else if (target.trySetStatus(status) && source.name === 'xjoelituh') {
						prompt = true;
					}
					if (prompt) {
